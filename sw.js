...
self.addEventListener("fetch", (event) => {
  ...
  if (request.mode === "navigate") {
    event.respondWith(estrategiaNetworkConCacheFallback(request));
    return;
  }

  if (new URL(request.url).origin !== self.location.origin) {
    event.respondWith(estrategiaCacheConNetworkFallback(request));
    return;
  }
...
async function estrategiaNetworkConCacheFallback(request) {
  try {
    ...
    try {
      ...
      console.warn(
        "No fue posible actualizar el caché estático con la respuesta de red.",
        errorCache,
      );
    }
